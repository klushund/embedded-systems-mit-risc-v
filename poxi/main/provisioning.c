...
#define WIFI_CONNECTED_EVENT					0x00000001

#define PROV_QR_VERSION         				"v1"
#define PROV_TRANSPORT_BLE      				"ble"
#define QRCODE_BASE_URL         				"https://espressif.github.io/esp-jumpstart/qrcode.html"

static EventGroupHandle_t gProvisioningEventGroup;
...
static void PrintQR(const char *name, const char *username, const char *pop, const char *transport);
...
static void eventHandler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    if (event_base == WIFI_PROV_EVENT) {
        ...
        xEventGroupSetBits(gProvisioningEventGroup, WIFI_CONNECTED_EVENT);
 ...
 */
esp_err_t customProvDataHandler(uint32_t session_id, const uint8_t *inbuf, ssize_t inlen,
...
void PrintQR(const char *name, const char *username, const char *pop, const char *transport) {
    if (!name || !transport) {
...
void provisioning_init() {
	...
	gProvisioningEventGroup = xEventGroupCreate();

	// Register our event handler for Wi-Fi, IP and Provisioning related events */
	ESP_ERROR_CHECK(esp_event_handler_register(WIFI_PROV_EVENT, ESP_EVENT_ANY_ID, &eventHandler, NULL));
	ESP_ERROR_CHECK(esp_event_handler_register(PROTOCOMM_TRANSPORT_BLE_EVENT, ESP_EVENT_ANY_ID, &eventHandler, NULL));
	...
	if (!provisioned) {
		...
		PrintQR(service_name, username, pop, PROV_TRANSPORT_BLE);
	} else {
	...
	xEventGroupWaitBits(gProvisioningEventGroup, WIFI_CONNECTED_EVENT, true, true, portMAX_DELAY);
}
